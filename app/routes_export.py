...
def get_current_list_phones(list_id: str) -> set:
    """Get all phone numbers currently in a ViciDial list."""
    try:
        conn = get_connection()
...
def get_excluded_phones(db, list_id: str, campaign_id: str, tenant_id: str) -> set:
    """Get phones that should be removed from the list (EXCLUDE status)."""
...
def get_new_skiptrace_leads(db, list_id: str, campaign_id: str, tenant_id: str, current_list_phones: set) -> list:
    """Get skip trace leads that are not already in the ViciDial list."""
...
def get_updated_data_records(db, date_from=None, date_to=None, campaign_id=None, list_id=None, tenant_id=None):
    """Legacy function kept for backward compatibility with /updated-data endpoint."""
    manual_exclusions = db.query(ManualExclusion).filter(ManualExclusion.tenant_id == tenant_id).all() if tenant_id else db.query(ManualExclusion).all()
    excluded_addresses = {normalize_address(e.address) for e in manual_exclusions if e.address}
...
def upload_to_vici(
    ...
    # Step 1 — Get current phones in ViciDial list
    current_phones = get_current_list_phones(list_id)

    if not current_phones:
        return JSONResponse(
            status_code=400,
            content={"error": "Lista vacia o no encontrada en ViciDial. Carga la lista primero."}
        )

    # Step 2 — Get phones to remove (EXCLUDE)
    ...
    # Step 3 — Get new skip trace leads to add
    ...
    if not phones_to_delete and not new_leads:
        return {
            "status": "ok",
            "message": "Lista ya esta al dia. No hay cambios necesarios.",
            "deleted": 0,
            "uploaded": 0,
            "failed": 0,
            "total": 0,
    ...
    # Step 4 — Delete excluded phones from ViciDial list
    deleted = 0
    if phones_to_delete:
        try:
            conn = get_connection()
            cursor = conn.cursor()
            placeholders = ','.join(['%s'] * len(phones_to_delete))
            cursor.execute(
                f"DELETE FROM vicidial_list WHERE list_id = %s AND phone_number IN ({placeholders})",
                [list_id] + list(phones_to_delete)
            )
            ...
            return JSONResponse(status_code=500, content={"error": f"Error borrando leads: {str(e)}"})

    # Step 5 — Upload new skip trace leads
    uploaded = 0
    failed = 0
    errors = []
    if new_leads:
